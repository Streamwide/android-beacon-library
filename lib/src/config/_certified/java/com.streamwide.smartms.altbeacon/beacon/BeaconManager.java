...
import com.streamwide.smartms.altbeacon.beacon.logging.Logger;
import com.streamwide.smartms.altbeacon.beacon.logging.Loggers;
import com.streamwide.smartms.altbeacon.beacon.service.BeaconService;
import com.streamwide.smartms.altbeacon.beacon.service.Callback;
...
import com.streamwide.smartms.altbeacon.beacon.service.RangedBeacon;
import com.streamwide.smartms.altbeacon.beacon.service.RegionMonitoringState;
import com.streamwide.smartms.altbeacon.beacon.service.RunningAverageRssiFilter;
import com.streamwide.smartms.altbeacon.beacon.service.ScanJobScheduler;
import com.streamwide.smartms.altbeacon.beacon.service.SettingsData;
import com.streamwide.smartms.altbeacon.beacon.service.StartRMData;
import com.streamwide.smartms.altbeacon.beacon.service.scanner.NonBeaconLeScanCallback;
import com.streamwide.smartms.altbeacon.beacon.simulator.BeaconSimulator;
...
public class BeaconManager {
    ...
    @NonNull
    protected final ConcurrentMap<BeaconConsumer, ConsumerInfo> consumers = new ConcurrentHashMap<>();
    ...
    @Nullable
    protected Boolean mScannerInSameProcess = null;
     ...
     */
    public static final long DEFAULT_BACKGROUND_BETWEEN_SCAN_PERIOD = 5 * 60 * 1000L;
    /**
    ...
    public void setBackgroundBetweenScanPeriod(long p) {
        ...
        if (backgroundBetweenScanPeriod < 15*60*1000 /* 15 min */) {
        ...
        synchronized (consumers) {
            ...
            if (alreadyBoundConsumerInfo != null) {
                ...
                if (mScheduledScanJobsEnabled) {
                    ...
                    if (this.getForegroundServiceNotification() != null) {
                        if (isAnyConsumerBound()) {
                            LogManager.i(TAG, "Not starting foreground beacon scanning" +
                                    " service.  A consumer is already bound, so it should be started");
                        }
                        else {
                            //FIXME : fix added for jira ticket SMARTMS-4128 to force the use of foreground service
    ...
    public void unbind(@NonNull BeaconConsumer consumer) {
        ...
        synchronized (consumers) {
            if (consumers.containsKey(consumer)) {
                ...
                if (consumers.size() == 0) {
                    //FIXME : fix added for an issue detected after app crash : beacon service is not stopped/released,
                    // and when app is restarted the beacon scan do not restart properly
                    try {
                        Intent intent = new Intent(consumer.getApplicationContext(), BeaconService.class);
                        mContext.stopService(intent);

                        LogManager.d(TAG, "FIXME#BEACON Stop BeaconService ");
                    } catch (SecurityException|IllegalStateException ex) {
                        LogManager.e(ex, TAG, "Exception when Stop BeaconService ");
    ...
    public void setRegionStatePersistenceEnabled(boolean enabled) {
        ...
        if (!isScannerInDifferentProcess()) {
            if (enabled) {
                MonitoringStatus.getInstance().startStatusPreservation(mContext);
            } else {
                MonitoringStatus.getInstance().stopStatusPreservation(mContext);
            }
        }
        this.applySettings();
    }
     ...
     * @return true if region state preservation is enabled
     */
     ...
     * @param region The region to request the state for
     */
    public void requestStateForRegion(@NonNull Region region) {
        if (determineIfCalledFromSeparateScannerProcess()) {
            return;
        }
        RegionMonitoringState stateObj = MonitoringStatus.getInstance().stateOf(mContext, region);
    ...
    public void startMonitoringBeaconsInRegion(@NonNull Region region) throws RemoteException {
        ...
        if (mScheduledScanJobsEnabled) {
            MonitoringStatus.getInstance().addRegion(mContext, region, new Callback(callbackPackageName()));
        }
        applyChangesToServices(BeaconService.MSG_START_MONITORING, region);

        if (isScannerInDifferentProcess()) {
            MonitoringStatus.getInstance().addLocalRegion(mContext, region);
        }
    ...
    public void stopMonitoringBeaconsInRegion(@NonNull Region region) throws RemoteException {
        ...
        if (mScheduledScanJobsEnabled) {
            MonitoringStatus.getInstance().removeRegion(mContext, region);
        }
        applyChangesToServices(BeaconService.MSG_STOP_MONITORING, region);
        if (isScannerInDifferentProcess()) {
            MonitoringStatus.getInstance().removeLocalRegion(mContext, region);
        }
    }
    ...
    public Collection<Region> getMonitoredRegions() {
        return MonitoringStatus.getInstance().regions(mContext);
    }
     ...
     */
    @Deprecated
    public static void logDebug(@Nullable String tag,@Nullable  String message) {
     ...
     */
    @Deprecated
    public static void logDebug(@Nullable String tag,@Nullable  String message,@Nullable Throwable t) {
     ...
     */
    @NonNull protected static Class rssiFilterImplClass = RunningAverageRssiFilter.class;
    ...
    @NonNull public static Class getRssiFilterImplClass() {
    ...
    public static void setBeaconSimulator(@NonNull BeaconSimulator beaconSimulator) {
        warnIfScannerNotInSameProcess();
    ...
    private class BeaconServiceConnection implements ServiceConnection {
        protected BeaconServiceConnection() {
        }
     ...
     * service
     */
    public void enableForegroundServiceScanning(@Nullable Notification notification, int notificationId)
     ...
     */
    @Nullable public Notification getForegroundServiceNotification() {
    ...
    private void setScheduledScanJobsEnabledDefault() {
        mScheduledScanJobsEnabled = true;
    }

    public void processBluetoothLeScanResults(Context context, ValidatingIntent validatingIntent ) {
        ...
        if (beaconManager.isAnyConsumerBound() || beaconManager.getScheduledScanJobsEnabled()) {
            try {
                ...
                int bleCallbackType = validatingIntent.getIntExtra(BluetoothLeScanner.EXTRA_CALLBACK_TYPE, -1); // e.g.
                ...
                if (bleCallbackType != -1) {
                    ...
                    int errorCode = validatingIntent.getIntExtra(BluetoothLeScanner.EXTRA_ERROR_CODE, -1); // e.g.
                    // ScanCallback.SCAN_FAILED_INTERNAL_ERROR
                    if (errorCode != -1) {
                        LogManager.w(TAG, "Passive background scan failed.  Code; " + errorCode);
                    }
                    ArrayList<ScanResult> scanResults =
                            validatingIntent.getParcelableArrayListExtra(BluetoothLeScanner.EXTRA_LIST_SCAN_RESULT);
                    if (scanResults != null) {
                        ScanJobScheduler.getInstance().scheduleAfterBackgroundWakeup(context, scanResults);
                    }

                } else if (validatingIntent.getBooleanExtra("wakeup", false)) {
                    LogManager.d(TAG, "got wake up intent");
                } else {
                    LogManager.d(TAG, "Already started.  Ignoring intent: %s of type: %s", validatingIntent,
                            validatingIntent.getStringExtra("wakeup"));
                }
            } catch (InvalidClassException e) {
                LogManager.d(TAG, "Invalid class exception.  Ignoring broadcast receiver.");
            }
        } else {

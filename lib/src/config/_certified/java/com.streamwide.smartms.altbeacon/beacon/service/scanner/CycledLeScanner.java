...
public abstract class CycledLeScanner {
     ...
     */
    @NonNull
    protected final HandlerThread mScanThread;
    ...
    protected CycledLeScanner(@NonNull Context context, long scanPeriod, long betweenScanPeriod, boolean backgroundFlag, @NonNull CycledLeScanCallback cycledLeScanCallback, @NonNull BluetoothCrashResolver crashResolver) {
    ...
    @NonNull
    public static CycledLeScanner createScanner(@NonNull Context context, long scanPeriod, long betweenScanPeriod, boolean backgroundFlag, @NonNull CycledLeScanCallback cycledLeScanCallback, @NonNull BluetoothCrashResolver crashResolver) {
     ...
     * @param backgroundFlag background mode flag
     */
    @MainThread
    ...
    @MainThread
    protected void scanLeDevice(final @NonNull Boolean enable) {
        try {
            ...
            if (mScanning) {
                if (getBluetoothAdapter().isEnabled()) {
                    ...
                    if (!mDistinctPacketsDetectedPerScan ||
                        ...
                        long now = SystemClock.elapsedRealtime();
                        if (mBetweenScanPeriod + mScanPeriod < ANDROID_N_MIN_SCAN_CYCLE_MILLIS && now - mLastScanCycleStartTime < ANDROID_N_MIN_SCAN_CYCLE_MILLIS) {
    ...
    protected void cancelAlarmOnUserSwitch() {
        if (mCancelAlarmOnUserSwitchBroadcastReceiver == null) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                mContext.registerReceiver(mCancelAlarmOnUserSwitchBroadcastReceiver, filter, RECEIVER_NOT_EXPORTED);
            } else {
    ...
    protected void cleanupCancelAlarmOnUserSwitch() {
        if (mCancelAlarmOnUserSwitchBroadcastReceiver != null) {
            try {
                ...
                // thrown if receiver was not registered
            } // thrown if OS does not think it was registered
    ...
    private boolean mustStopScanToPreventAndroidNScanTimeout() {
        long timeOfNextScanCycleEnd = SystemClock.elapsedRealtime() +  mBetweenScanPeriod +
                mScanPeriod;
        boolean timeoutAtRisk = mCurrentScanStartTime > 0 && timeOfNextScanCycleEnd - mCurrentScanStartTime > ANDROID_N_MAX_SCAN_DURATION_MILLIS;

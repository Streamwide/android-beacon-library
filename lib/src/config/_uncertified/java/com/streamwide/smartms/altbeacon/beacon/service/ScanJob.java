...
public class ScanJob extends JobService {
    protected static final String TAG = ScanJob.class.getSimpleName();
    /*
    ...
    @Nullable
    protected ScanState mScanState = null;
    @NonNull
    protected Handler mStopHandler = new Handler();
    @Nullable
    protected ScanHelper mScanHelper;
    protected boolean mInitialized = false;
    protected boolean mStopCalled = false;

    @Override
    public boolean onStartJob(final @NonNull JobParameters jobParameters) {
        ...
        new Thread(new Runnable() {
            public void run() {
                ...
                if (jobParameters.getJobId() == getImmediateScanJobId(ScanJob.this)) {
                    LogManager.d(TAG, "Running immediate scan job: instance is " + ScanJob.this);
                } else {
                    LogManager.d(TAG, "Running periodic scan job: instance is " + ScanJob.this);
                }
                ...
                synchronized (ScanJob.this) {
                    ...
                    if (startedScan) {
                        if (mScanState != null) {
                            LogManager.d(TAG, "Scan job running for " + mScanState.getScanJobRuntimeMillis() + " millis");
                            mStopHandler.postDelayed(new Runnable() {
                                @Override
                                public void run() {
                                    LogManager.d(TAG, "Scan job runtime expired: " + ScanJob.this);
                                    stopScanning();
                                    mScanState.save();
                                    ScanJob.this.jobFinished(jobParameters, false);
    ...
    private void startPassiveScanIfNeeded() {
        if (mScanState != null) {
            ...
            List<Region> regions = new ArrayList<>(mScanState.getMonitoringStatus().regions(ScanJob.this));
            for (Region region : regions) {
                RegionMonitoringState state = mScanState.getMonitoringStatus().stateOf(ScanJob.this, region);
    ...
    protected boolean initialzeScanHelper() {
    ...
    protected boolean restartScanning() {
        if (mScanState != null && mScanHelper != null) {
            ...
            if (mScanHelper.getRangedRegionState().size() > 0 || mScanHelper.getMonitoringStatus().regions(ScanJob.this).size() > 0) {
                if (mScanHelper.getCycledScanner() != null) {
    ...
    protected boolean startScanning() {
        ...
        if (beaconManager.isMainProcess()) {
            LogManager.i(TAG, "scanJob version %s is starting up on the main process");
        } else {
            LogManager.i(TAG, "beaconScanJob library version %s is starting up on a separate process");
        ...
        ModelSpecificDistanceCalculator defaultDistanceCalculator = new ModelSpecificDistanceCalculator(ScanJob.this);
     ...
     * @return the job id
     */
    public static int getImmediateScanJobId(@NonNull Context context) {
     ...
     * @return the job id
     */
    public static int getPeriodicScanJobId(@NonNull Context context) {
        if (sOverrideImmediateScanJobId >= 0) {

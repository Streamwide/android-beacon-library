...
import static com.streamwide.smartms.altbeacon.util.ExtensionsKt.registerReceiverByVersion;
...
public class BluetoothCrashResolver {
    ...
    protected boolean discoveryStartConfirmed = false;

    protected long lastBluetoothOffTime = 0l;
    protected long lastBluetoothTurningOnTime = 0l;
    ...
    public void start() {
        ...
        registerReceiverByVersion(context, receiver, filter);
     ...
     * @param device
     */
    public void notifyScannedDevice(@NonNull BluetoothDevice device, @NonNull BluetoothAdapter.LeScanCallback scanner) {
        int oldSize, newSize;
        ...
        if (distinctBluetoothAddresses.size() > getCrashRiskDeviceCount()) {
            if (PREEMPTIVE_ACTION_ENABLED && !recoveryInProgress) {
                ...
                try {
                    ...
                    LogManager.e(ex, TAG, "SecurityException on stopping LE scan");
                }
                startRecovery();
                processStateChange();
    ...
    public void setUpdateNotifier(@NonNull UpdateNotifier updateNotifier) {
    ...
    private void startRecovery() {
        try {
            ...
            LogManager.e(ex, TAG, "Security Exception when starting recovery");
        }
    ...
    private void cancelDiscovery() {
        try {
            ...
            LogManager.e(TAG, "DiscoveryCanceller sleep interrupted.", e);
